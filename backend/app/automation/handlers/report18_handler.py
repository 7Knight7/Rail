...
class Report18Handler(BaseReportHandler):
    ...
    async def execute(
        ...
        if not await self.generator.verify_report_displayed(report_root):
            # One Playwright-driven recheck after loaders settle.
            try:
            ...
            if not await self.generator.verify_report_displayed(report_root):
                return self.build_failed_result(
                    report.slug,
                    "REPORT18_SUBMIT_FAILED: report table did not display after Submit",
                )

        # --- Validation: table exists + at least one data row ---
        ...
        if not validation.valid:
            return self.build_failed_result(
                report.slug,
                f"REPORT18_TABLE_INVALID: {validation.error or 'table missing or empty'}",
            )

        csv_path = extracted_dir / REPORT18_CSV_FILENAME
        ...
        row_counts = {"vande_bharat": data_rows}
